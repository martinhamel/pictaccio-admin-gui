 ...
 */
export class NumberFilterFunction extends FilterFunction<number[]> {
    /**
     * Receive the value of the filter
     * @param value is the value of the filter
     * @param column is the column of the table
     */
    constructor(value: number | number[], type: FilterType, column: string) {
    ...
    public createRemoteFilters(builder: FilterBuilder): void {
        builder.addFilter({ column: this.column, operator: 'IN' as Operator, operand: this.value }, this.column);
    }
