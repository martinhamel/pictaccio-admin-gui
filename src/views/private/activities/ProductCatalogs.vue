...
<template>
    <section id="categories" class="activity-wrapper">
        <crud-dialog-catalog ref="crud-dialog"
                             @update="updateDatatable"/>
        <activities-tabs ref="tabs"/>
        <div ref="product-catalogs-container"
             class="activity-control table-holder">
            <data-table ref="product-catalogs-table"
                        name="product-catalogs"
                        :tableData="productCatalogs"
                ...
                <template #placeholder>
                    <placeholder-data-table :headers="productCatalogs.headers"
                                            :rows="6"
                                            @placeholder-clicked="addProductCatalogsClick"/>
                </template>
                <template #tutorial>
<!--                    <tutorial-text ref="add-btn-tutorial"-->
<!--                                   v-if="$validateCapability('product:create')"-->
<!--                                   :name-ref="'product-catalogs-add-btn-tutorial'"-->
...
<!--                        {{ $t('activities.productCatalogs.tutorial.addBtn') }}-->
<!--                    </tutorial-text>-->
<!--                    <tutorial-text ref="product-catalogs-container-tutorial"-->
<!--                                   :name-ref="'product-catalogs-container-tutorial'"-->
<!--                                   :background-shape="'blob-h-1'"-->
<!--                                   :ui-offset="{ ui: '0 0', next: '1rem 1rem', skip: '5.5rem -1.5rem' }"-->
<!--                                   :orientation="'top-right_down'"-->
<!--                                   @mounted="getTutorialTargetPosition('product-catalogs-container',-->
<!--                                   '.table-products, .placeholder-container')">-->
<!--                        {{ $t('activities.productCatalogs.tutorial.whatIs') }}-->
<!--                    </tutorial-text>-->
                </template>
...
export default class ProductCatalogs extends AdminActivities {
    private productCatalogs = PRODUCT_CATALOG_COLUMNS;
    private crudDialog: CrudDialogCatalog;
    ...
    public async mounted(): Promise<void> {
        super.mounted();
        (this.$refs['product-catalogs-table'] as DataTable).setRemoteStore(
            new RemoteStore<DbProductCatalog>('ProductCatalogs'), !shouldExtraSilenceRead(this.extra));
        this.crudDialog = (this.$refs['crud-dialog'] as CrudDialogCatalog);

        await this.$store.dispatch('ProductsReader/productsRead');
        (this.$refs['product-catalogs-table'] as DataTable).applyGetters({
            products: this.$store.getters['ProductsReader/products']
                .map(i => ({ id: i.id, value: i.value }))
        });

        pushBreadcrumb({
            id: 'level2',
            display: 'activities.tabsTitles.productCatalogs',
            routerLink: '/activities/product-catalogs'
        });
        this.createFilters();
    }

    /* EVENT HANDLERS */
    private addProductCatalogsClick(): void {
    ...
    private applyFilters(filters: FilterFunction[]): void {
        ...
        (this.$refs['product-catalogs-table'] as DataTable).applyFilters(filters);
    }
    ...
    private async updateDatatable(): Promise<void> {
        await (this.$refs['product-catalogs-table'] as DataTable).refresh();
    }
}
</script>
